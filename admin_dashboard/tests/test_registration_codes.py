...
def async_task():
    ...
    try:
        result = call_realm_extension("admin_dashboard", "generate_registration_url", {
        ...
        if result.get("success"):
            ...
            print_error(f"✗ Failed to generate registration code: {result.get('error')}")
            generated_code = None
    ...
    if generated_code:
        ...
        try:
            result = call_realm_extension("admin_dashboard", "validate_registration_code", {
                "code": generated_code
            })
    ...
    try:
        result = call_realm_extension("admin_dashboard", "get_registration_codes", {
            "include_used": True
        })
    ...
    return {
        "success": True,
        "message": "Registration code tests completed"
    }
