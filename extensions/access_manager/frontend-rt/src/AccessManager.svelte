<script lang="ts">
	...
	function isAccessPermissionError(error: string | undefined | null): boolean {
		if (!error) return false;
		const e = error.toLowerCase();
		return (
			e.includes('access denied') ||
			e.includes('lacks permission') ||
			e.includes('permission denied') ||
			e.includes('user') && e.includes('not found')
		);
	}
	...
	async function loadDepartments() {
		...
		try {
			...
			if (!res?.success) {
				...
				selectedDeptName = '';
				authorities = [];
				if (isAccessPermissionError(err)) {
