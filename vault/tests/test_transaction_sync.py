...
def test_single_deposit_creates_entities() -> bool:
    ...
    try:
        ...
        transfers = query_ggg_entities("Transfer", page_num=0, page_size=10, order="desc")
        ...
        if transfer_count <= initial_transfer_count:
            print_error(f"Transfer count did not increase: {initial_transfer_count} -> {transfer_count}")
            return False
        
        print_ok(f"Transfer count increased: {initial_transfer_count} -> {transfer_count}")
        ...
        if transfer_amount != amount:
            print_error(f"Transfer amount mismatch: expected {amount}, got {transfer_amount}")
            checks_passed = False
        else:
            print_ok(f"Transfer amount correct: {amount}")
        
        if transfer_from != sender_principal:
            print_error(f"Transfer sender mismatch: expected {sender_principal}, got {transfer_from}")
            checks_passed = False
        else:
            print_ok("Transfer sender correct")
        
        if transfer_to != realm_backend_id:
            print_error(f"Transfer receiver mismatch: expected {realm_backend_id}, got {transfer_to}")
            checks_passed = False
        else:
        ...
        balances = query_ggg_entities("Balance", page_num=0, page_size=100, order="desc")
...
def test_multiple_deposits_accumulate_balance() -> bool:
    ...
    try:
        ...
        if final_balance_amount != expected_balance:
            print_error(f"Balance mismatch: expected {expected_balance}, got {final_balance_amount}")
            return False
...
def test_duplicate_sync_skips_existing() -> bool:
    ...
    try:
        ...
        new_txs_1 = refresh1.get("data", {}).get("TransactionSummary", {}).get("new_txs_count", 0)
        ...
        new_txs_2 = refresh2.get("data", {}).get("TransactionSummary", {}).get("new_txs_count", 0)
        ...
        if new_txs_2 > 0:
            print_error(f"Second refresh created {new_txs_2} transactions (should be 0)")
            return False
        
        if count_after_second != count_after_first:
            print_error(f"Transfer count changed on second refresh: {count_after_first} -> {count_after_second}")
            return False
...
def test_withdrawal_decreases_balance() -> bool:
    ...
    try:
        ...
        if not transfer_result or not transfer_result.get("success"):
            error = transfer_result.get("error", "Unknown") if transfer_result else "Call failed"
        ...
        print_ok(f"✅ Balance correctly decreased: {initial_balance} -> {final_balance}")
        return True
...
def test_transaction_data_integrity() -> bool:
    ...
    try:
        ...
        transfers = query_ggg_entities("Transfer", page_num=0, page_size=5, order="desc")
        ...
        required_fields = ["id", "principal_from", "principal_to", "amount", "timestamp"]
        all_valid = True
